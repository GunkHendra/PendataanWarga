<nav class="w-72 bg-gray-900 min-h-screen text-white flex flex-col sticky top-0 print:hidden shadow-lg">
    ...
    <div class="p-6 text-center items-center flex flex-col space-y-4 border-b-2 border-blue-400">
        ...
        <div>
            <h1 class="text-lg font-semibold">{{ $user->nama_lengkap }}</h1>
            <h2 class="text-sm">Warga Desa {{ $desa->desa }}</h2>
        </div>
    </div>

    <!-- Menu Navigasi -->
    <div class="flex-1 px-4 py-6 space-y-2">
        <a href="/user" class="{{ ($title === 'Dashboard') ? 'bg-blue-600 shadow-md' : 'hover:bg-blue-900' }} flex items-center p-3 rounded-lg transition duration-300">
            <img class="w-6 h-6 mr-3" src="/assets/dashboard-icon/home.png" alt="Home Icon">
            <span class="text-sm">Dashboard</span>
        </a>
        <a href="/user/riwayat_iuran" class="{{ ($title === 'Riwayat Iuran') ? 'bg-blue-600 shadow-md' : 'hover:bg-blue-900' }} flex items-center p-3 rounded-lg transition duration-300">
            <img class="w-6 h-6 mr-3" src="/assets/dashboard-icon/person-warga.png" alt="Person Icon">
            <span class="text-sm">Riwayat Iuran</span>
        </a>
